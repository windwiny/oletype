...
class Cfg():
    ...
    OBJECT_PROPERTY = [repr(i) for i in dir(object())]
...
def check_or_exit():
    if not os.path.exists(Cfg.OUT_EXCEL_INFO_FN):
        print(f'not exists method comment file {Cfg.OUT_EXCEL_INFO_FN}')
        sys.exit(2)
...
def get_cln_method_return(cln: str,
    ...
    if objinfo:
        ...
        if objtype == 'None' or objtype is None:
            print(f'EEEE2 {objinfo}', file=sys.stderr) # TODO1
            return None
...
def get_cln_property_type(cln: str,
    ...
    if objinfo:
        ...
        if '.' in ty:
            ty = ty.split('.')[-1]
        if not ty in Cfg.BUILTINS_CLASS and not ty in all_see_cls:
            all_see_cls.add(ty)
        return ty
    return None
...
def conv2cls(ex: CDispatch,
    ...
    cls_comment = get_cln_class_comment(cls_name, ole_info_kvs)
    if cls_comment:
    ...
    for ix in o_attrs:
        ...
        if ty:
            ...
            ty = '__TYPE_UNKNOWN'
            all_see_cls.add(ty)
        comment_or_pass = get_cln_property_comment(cls_name, na, ole_info_kvs, all_see_cls)
    ...
    for ix in o_methods:
        ...
        if ret is not None:
            ...
            rets = ''
        comment_or_pass = get_cln_method_comment(cls_name, na, ole_info_kvs)
    ...
    for ix in e_ee:
        ...
        print(f"    # {na}:  {ee}", file=ff)
    print(file=ff)
    print(f'# Summary "{ex.__class__.__mro__}", attrs:{len(o_attrs)}, methods:{len(o_methods)}, whats:{len(o_unknowns)},   ok:{len(o_attrs) + len(o_methods) + len(o_unknowns)}, e_noattr:{len(e_noattr)}, e_eerror:{ len(e_ee)}', file=ff)
    return ff.getvalue()
...
def showinfo(cls_name: str,
    ...
    e_ee = []
    for i in xx:
        if repr(i) in Cfg.OBJECT_PROPERTY:
            continue
        try:
            # ex.__getattr__(i)
            vv = getattr(ex, i)
...
def output_all_cls_pyi(all_cls_kvs: dict[str, any],
                       unparsed_cls: set,
                       ole_cls_def_ns: set,
...
def runit():
    ...
    output_all_cls_pyi(all_cls_kvs, unparsed_cls, ole_cls_def_ns, ole_cls_defstrs, all_see_cls, ole_info_kvs)
    output_all_cls_pyfake(Cfg.OUT_EXCEL_PY_FN, all_cls_kvs, ole_cls_def_ns, ole_info_kvs)
